...
bf_round proc
    ; l ^= key->p[i];
    xor    l, [rsi][BF_KEY.p][4*rbp]
    mov    eax, l
    rol    eax, 16
    ; t  = key->sbox1[((l >> 24) & 0xFF)];
    movzx  edx, ah
    mov    t, [rsi][BF_KEY.sbox1][4*rdx]
    ; t += key->sbox2[((l >> 16) & 0xFF)];
    mov    dl, al
    add    t, [rsi][BF_KEY.sbox2][4*rdx]
    ; t ^= key->sbox3[((l >>  8) & 0xFF)];
    mov    dl, bh
    xor    t, [rsi][BF_KEY.sbox3][4*rdx]
    ; t += key->sbox4[((l      ) & 0xFF)];
    mov    dl, bl
    add    t, [rsi][BF_KEY.sbox4][4*rdx]
    xor    r, t
    xchg   r, l
    ret
bf_round endp
...
load_key_word:
    ...
    ; ==============================
    xor    eax, eax
    push   rax       ; in[0] = 0, in[1] = 0
    mov    rsi, rsp
    mov    ecx, sizeof (BF_KEY) / 8
encrypt_key:
    push   rcx
...
encrypt_loop:
    ...
    xor    l, [rsi][BF_KEY.p][BF_ROUNDS*4]
    xor    r, [rsi][BF_KEY.p][(BF_ROUNDS+1)*4]
...
decrypt_loop:
    ...
    xor    r, [rsi][BF_KEY.p][0]
    xor    l, [rsi][BF_KEY.p][4]

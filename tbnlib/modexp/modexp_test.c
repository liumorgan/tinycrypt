...
void dh_asm (uint32_t numlen, void *p, void *g, void *x, void *y)
{
  ...
  dh.maxbytes=numlen>>3;
  dh.maxbits=(numlen&-32)+32;
  
  // group parameters
  memcpy (dh.p, p, dh.maxbytes);
  memcpy (dh.g, g, 1);             // this is presumed to be number 2
  
  // then private keys
  memcpy (dh.x, x, dh.maxbytes);
  memcpy (dh.y, y, dh.maxbytes);
  
  // Alice obtains A = g ^ x mod p
  modexp (dh.maxbits, dh.g, dh.x, dh.p, dh.A);
  
  // Bob obtains B = g ^ y mod p
  modexp (dh.maxbits, dh.g, dh.y, dh.p, dh.B);
  ...
  modexp (dh.maxbits, dh.B, dh.x, dh.p, dh.s1);
  
  // Bob computes   s2 = A ^ y mod p
  modexp (dh.maxbits, dh.A, dh.y, dh.p, dh.s2);
  
  // s1 + s2 should be equal
  if (memcmp (dh.s1, dh.s2, dh.maxbytes)==0) {
    printf ("\n\nx86 asm Key exchange succeeded");
    dump_bn ("\n\nx86 Session key = ", dh.s1, dh.maxbytes);
  } else {

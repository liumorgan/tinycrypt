...
typedef struct {
      ...
      unsigned int  datasize_in_buffer;    /*the size of the message remained in buffer; 
                                           // assumed to be multiple of 8bits except for the last 
                                           // partial block at the end of the message*/
...
const unsigned char roundconstant_zero[32] = 
{0x6a,0x9,0xe6,0x67,0xf3,0xbc,0xc9,0x8,0xb2,0xfb,0x13,0x66,0xea,0x95,0x7d,0x3e,
 0x3a,0xde,0xc1,0x75,0x12,0x77,0x50,0x99,0xda,0x2f,0x59,0xb,0x6,0x67,0x32,0x2a};
...
*/
unsigned char SboxMDS_A[2][16] = 
{{0xb1,0x0,0x78,0x15,0xc9,0x9b,0xf6,0x6d,0x52,0x47,0xa4,0xdc,0x8e,0x2a,0xe3,0x3f}, 
 {0xf6,0x9b,0xdc,0xc9,0x2a,0x8e,0x52,0xb1,0x6d,0xa4,0x0,0x78,0x15,0x47,0x3f,0xe3}};
 
unsigned char SboxMDS_B[2][16] = 
{{0x19,0x0,0x84,0x5b,0x9d,0xbc,0x63,0xdf,0x21,0x7a,0x42,0xc6,0xe7,0xa5,0x38,0xfe}, 
 {0x63,0xbc,0xc6,0x9d,0xa5,0xe7,0x21,0x19,0xdf,0x42,0x0,0x84,0x5b,0x7a,0xfe,0x38}};
...
void update_roundconstant(hashState *state) {
      ...
      /* S box layer and MDS layer */
      for (i = 0; i < 32; i++) 
        tem[i] = SboxMDS_A[0][state->roundconstant[i] >> 4] ^ SboxMDS_B[0][state->roundconstant[i] & 0xf]; /*constant bits are set as 0*/
...
HashReturn Final(hashState *state, BitSequence *hashval)
{
      ...
      if ( (state->databitlen & 0x1ff) == 0) {
            ...
            state->buffer[0] = 0x80;
            for (i = 0; i < 8; i++) 
              state->buffer[63-i] = (state->databitlen >> (i << 3)) & 0xff;
            F8(state);
      }
      else {
			...
			if ( (state->datasize_in_buffer & 7) == 0)
               ...
               i = ((state->databitlen & 0x1ff) >> 3)+1;
               memset(state->buffer[i], 0, 64 - i);
            }

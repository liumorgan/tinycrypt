...
%define _a qword[ebx+0*8]
%define _b qword[ebx+1*8]
%define _c qword[ebx+2*8]
%define _d qword[ebx+3*8]
%define _e qword[ebx+4*8]
%define _f qword[ebx+5*8]
%define _g qword[ebx+6*8]
%define _h qword[ebx+7*8]
...
expand:
    ...
    paddq  s0, qword[8*i+edi-16*8]
    paddq  s1, qword[8*i+edi-7*8]
    paddq  s0, s1
    movq   qword[edi+8*i], s0
    inc    i
    cmp    i, 80
    jnz    expand
...
update_loop:
    ...
    paddq  t1, s1
    paddq  t1, _h
    paddq  t1, qword[esi+8*i] ; _k[i]
    paddq  t1, qword[edi+8*i] ; w[i]
    ; S0 := EP0(a)
